...
import {
    base64Encode, isAscii, Logger, resolve, throwError
} from './utilitas.mjs';

import {
    error, querySecurityLog as _querySecurityLog, route,
    WEB_AUTH_ADD, WEB_BYPASS_ADD
} from './web.mjs';
...
const onConnect = async (clientSocket, connections, options, logger) => {
    ...
    const querySecurityLog = () => _querySecurityLog(clientSocket.remoteAddress);
    ...
    const setSecurityLog = (user) => {
        ...
        return process?.send?.(
            { action: WEB_AUTH_ADD, data: clientSocket.remoteAddress }
        );
    };
    ...
    const prepareTunnel = async (data, firstHeaderRow, isConnectMethod = 0) => {
        ...
        const onOpen = async (connectionError) => {
            ...
            options?.bypass?.has?.(country)
                && process.send({ action: WEB_BYPASS_ADD, data: ADDRESS });
    ...
    const onDataFromClient = async (data) => {
        ...
        try {                        // @todo: make secure, split can be limited
            ...
            if (isAuthenticated()) {
                ...
                (await resolve(basicAuth(user, passwd, tunnel))) || throwUnauted();
