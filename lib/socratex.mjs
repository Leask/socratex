...
const onConnect = async (clientSocket, connections, options, logger) => {
    ...
    const setSecurityLog = async (user, lastVisit) => {
        querySession().setUserAuthentication(user);
        return new Date() - lastVisit > SESSION_SYNC_INTERVAL
            ? await _setSecurityLog(clientSocket.remoteAddress) : true;
    };
    ...
    const onDataFromClient = async (data) => {
        ...
        try {                        // @todo: make secure, split can be limited
            ...
            if ((lastVisit = await isAuthenticated())) {
                await setSecurityLog('[TRANSFERRED]', lastVisit);
