import { route, error } from './web.mjs';
...
const onConnect = async (
    ...
    const prepareTunnel = async (data, firstHeaderRow, isConnectMethod = 0) => {
        ...
        const handleHeaders = (data, connectionOpt) => {
            ...
            return rebuildHeaders(headers, data);
        }

        const onHTTPConnectOpen = async (connectionError) => {
            if (connectionError) { return await onClose(connectionError); }
            if (connectionOpt.credentials) {
                tunnel.clientRequestWrite(handleHeaders(data, connectionOpt));
            } else { onDirectConnectionOpen(data); }
        };

        const onHTTPSConnectOpen = async (connectionError) => {
            if (connectionError) { return await onClose(connectionError); }
            if (!connectionOpt.upstreamed) { // response as normal http-proxy
                tunnel.clientResponseWrite(OK + CLRF + CLRF);
                return await updateSockets();
            }
            if (connectionOpt.credentials) {
                tunnel.clientRequestWrite(handleHeaders(data, connectionOpt));
            } else { onDirectConnectionOpen(data); }
        ...
        if (connectionOpt?.host && connectionOpt?.port) {
            ...
            logger.log(`${getStatus()}=> ${ADDRESS}:${PORT}`);
    ...
    constructor(options) {
        ...
        if (Function.isFunction(options.setStatus)) {
            ...
            const cleanSecurityLog = async () => {
                ...
                logger.log(
                    `Saved ${Object.keys(securityLog).length} session(s).`
                );
            };
            setInterval(async () => {
