import { bypass, error, route } from './web.mjs';
...
import {
    base64Encode, ignoreErrFunc, isAscii, Logger, resolve, throwError
} from './utilitas.mjs';
...
const onConnect = async (
    ...
    const prepareTunnel = async (data, firstHeaderRow, isConnectMethod = 0) => {
        ...
        const onOpen = async (connectionError) => {
            ...
            options?.bypass?.has?.(country) && bypass(ADDRESS);
...
class Socrates extends createServer {
    ...
    constructor(options) {
        ...
        Function.isFunction(options.getStatus) && (async () => {
            ...
            logger.log(`Restored ${Object.keys(securityLog).length} session(s).`);
        })();
