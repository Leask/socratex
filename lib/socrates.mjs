import { __, isAscii, Logger, which, modLog, maskPassword } from './utilitas.mjs';
import net from 'net';
...
const init = async (options) => {
    ...
    if (options.user && options.password) {
        options.auth = (username, password) => {
            ...
            return username === options.user && password === options.password;
        };
    }
    if (!options.port) {
    ...
    socrates.listen(options.port, options?.listen || '', async () => {
        ...
        modLog(`Server started at ${add}.`, `${meta?.title}.x`);
    });
