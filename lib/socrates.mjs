import {
    __, insensitiveCompare, isAscii, log, Logger, maskPassword, throwError,
    which,
} from './utilitas.mjs';
...
import {
    ...
    SLASH_REGEXP, SLASH, STATUS, STRINGS,
} from './consts.mjs';
...
class Socrates extends net.createServer {
    ...
    constructor(options) {
        ...
        const connections = {};
        super(clientSocket => {
            onConnect(clientSocket, connections, opts, logger)
        });
...
const init = async (options) => {
    ...
    if (options.user && options.password) {
        options.auth = (username, password) => {
            log(
                `Authenticate: ${username}:${maskPassword(password)}.`,
                meta?.name, logWithTime
            );
            return insensitiveCompare(username, options.user)
                && password === options.password;
        };
    }
    if (!options.port) {
        options.port = options.httpOnly ? HTTP_PORT : HTTPS_PORT;
    }
    if (!options.domain) {
        options.domain = DEFAULT_OPTIONS.domain;
    }
    const protocol = `${options.httpOnly ? HTTP : HTTPS}://`;
    ...
    socrates.listen(options.port, options?.listen || '', async () => {
        ...
        const add = `${protocol}${options.domain}:${port} (${family} ${address})`;
        log(`Server started at ${add}.`, `${meta?.title}.x`);
    });
