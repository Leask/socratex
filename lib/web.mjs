import { basename, getLogCache, log as _log, resolve } from './utilitas.mjs';
...
function FindProxyForURL(url, host) {
    ...
    let lcl = ['127.0.0.1', '10.*.*.*', '172.16.*.*', '192.168.*.*', '*.local'];
    for (let item of lcl) { if (shExpMatch(host, item)) { return 'DIRECT'; } }
...
const bypass = async (host) => {
    // process.send()
    bypassList[host] = { lastVisit: new Date() };
};
...
const init = async (options) => {
    ...
    return await loop(
        cleanBypassList, 60, 60, 0, basename(import.meta.url), { silent: true }
    );
};
