...
import { MAX_LOG_CACHE, get as getLog } from './log.mjs';
...
const {
    AUTH_REQUIRED, ENOTFOUND, EPIPE, EPROTO, ETIMEDOUT, NOT_FOUND, NOT_OK, OK,
    TIMED_OUT, UNAUTHORIZED,
} = HTTP_RESPONSES;
...
const makeLog = (log) => [
    ...
    '    <pre>',
    `============ Socratex Console (latest ${MAX_LOG_CACHE} logs) ============`,
    '...',
    ...utilitas.renderCode([...log], { asArray: true }),
    '    </pre>',
    '</body>',
    '</html>',
].join('\n');
...
const route = async (method, path, protocol, req) => {
    const tokenQuery = globalThis._socratex?.token ? `?token=${_socratex.token}` : '';
    switch (`${String(method).toUpperCase()} ${String(path).toLowerCase()}`) {
        ...
        case `${GET} /log`:
        case `${GET} /log${tokenQuery}`:
            return packResult(OK, makeLog(await getLog()));
        case `${GET} /proxy.pac`:
        case `${GET} /proxy.pac${tokenQuery}`:
        case `${GET} /wpad.dat`:
        case `${GET} /wpad.dat${tokenQuery}`:
            return packResult(`${OK}${CLRF}${HEADERS.PAC}`,
