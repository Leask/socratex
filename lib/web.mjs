...
const castBypassList = () => _socrates.processes.map(x => x.send({
    action: WEB_BYPASS_UPDATE, data: getBypassList()
}));

const castSecurityLog = () => _socrates.processes.map(x => x.send({
    action: WEB_AUTH_UPDATE, data: securityLog,
}));
